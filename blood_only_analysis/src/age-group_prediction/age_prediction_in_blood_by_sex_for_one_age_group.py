...
# age_groups
age_groups = ["infant","young_child","child","adolescent","young_adult","adult","middle_adult","older_adult","old_adult"]
...
for fold in nfolds:
  ...
  for ag in age_groups:
    tst_lab = np.where(tst_labels.age_group == ag, True, False).tolist()
    auprc = average_precision_score(tst_lab, probs)
    alt_ag_auprcs.append(auprc)
  
  model_weights.append(weights)
  auroc_results.append(alt_ag_aurocs)
  auprc_results.append(alt_ag_auprcs)
  # sample probs
...
# write results  
results_path = "~/projects/age-sex-prediction/blood-only_analysis/results/age_prediction_by_sex/"+ args.data_type + "/output_files/"
...
auprc_results = pd.DataFrame(auprc_results, columns = age_groups)
auprc_results['n_positives'] = positives
sample_probs = pd.concat(sample_probs_df_list, ignore_index=True)

if args.data_type == "microarray":
  auroc_results.to_csv(results_path + "3FCV_elasticnet_LR_auroc_no_std_scaling_" + args.sex + "_" + args.pos_age_group + "_results_with_fine_ags.tsv", sep = "\t", header = True, index = False)
  model_weights.to_csv(results_path + "3FCV_elasticnet_LR_model_weights_no_std_scaling_" + args.sex + "_" + args.pos_age_group + "_weights_with_fine_ags.tsv", sep = "\t", header = True, index = False)
  sample_probs.to_csv(results_path + "3FCV_elasticnet_LR_sample_probabilities_no_std_scaling_" + args.sex + "_" + args.pos_age_group + "_results_with_fine_ags.tsv", sep = "\t", header = True, index = False)
  auprc_results.to_csv(results_path + "3FCV_elasticnet_LR_auprc_no_std_scaling_" + args.sex + "_" + args.pos_age_group + "_results_with_fine_ags.tsv", sep = "\t", header = True, index = False)

if args.data_type == "rnaseq":
  auroc_results.to_csv(results_path + "3FCV_elasticnet_LR_auroc_asinh_transformed_no_std_scaling_" + args.sex + "_" + args.pos_age_group + "_results_with_fine_ags.tsv", sep = "\t", header = True, index = False)
  model_weights.to_csv(results_path + "3FCV_elasticnet_LR_model_weights_asinh_transformed_no_std_scaling_" + args.sex + "_" + args.pos_age_group + "_weights_with_fine_ags.tsv", sep = "\t", header = True, index = False)
  sample_probs.to_csv(results_path + "3FCV_elasticnet_LR_sample_probabilities_asinh_transformed_no_std_scaling_" + args.sex + "_" + args.pos_age_group + "_results_with_fine_ags.tsv", sep = "\t", header = True, index = False)
  auprc_results.to_csv(results_path + "3FCV_elasticnet_LR_auprc_asinh_transformed_no_std_scaling_" + args.sex + "_" + args.pos_age_group + "_results_with_fine_ags.tsv", sep = "\t", header = True, index = False)
