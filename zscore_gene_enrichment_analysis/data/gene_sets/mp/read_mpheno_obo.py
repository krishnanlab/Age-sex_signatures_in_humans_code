...
'''
This was mdified from embed_ontologies
'''
...
if __name__ == '__main__':
    ...
    fp = '/mnt/research/compbio/krishnanlab/data/ontologies/mammalian_phenotype_ontology/'
    FN = 'MPheno_OBO.ontology'

        
    terms = get_terms(fp+FN)
    ...
    with open(fp+'mp_terms.tsv', 'w') as f:
        for ID in ID_name_map:
            f.write('{}\t{}\n'.format(ID, ID_name_map[ID]))

    with open(fp+'mp_ancestors.tsv', 'w') as f:
        for node in graph.nodes():
            ancestors = nx.ancestors(graph, node)
            if (len(ancestors) > 0):
                f.write('{}\t{}\n'.format(node, ', '.join(ancestors)))

    with open(fp+'mp_descendants.tsv', 'w') as f:
        for node in graph.nodes():
