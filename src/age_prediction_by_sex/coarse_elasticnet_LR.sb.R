...
age_groups <- c("fetus","infant","child","adolescent","adult","older_adult","elderly")

src_dir <- "/mnt/home/john3491/projects/age-sex-prediction/src/age_prediction_by_sex/"

# make a directory to put sbatches
dirname <- paste0("./sbatches_age_prediction_by_sex_coarse_elasticnet")
...
for (sex in c("female", "male")){
  for (dt in c("rnaseq", "microarray")){
    for (ss in c("with", "without")){
      ...
      #make sbatch
      rjobsh <- paste0("coarse_el_", sex, "_", ag, "_", dt, "_", ss, "_stdsclng", ".rjob.sh"); cat(rjobsh, "\n")
