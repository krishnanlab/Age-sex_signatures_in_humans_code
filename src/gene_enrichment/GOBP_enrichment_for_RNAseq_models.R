...
common_genes = read_delim("~/projects/age-sex-prediction/data/refine.bio/common_Entrez_IDs_gpl570-refine.bio.txt", 
                          delim = "\t", col_names = F) %>% 
  pull(1)
rnaseq_genes <- read_delim("~/projects/age-sex-prediction/data/gene_enrichment_for_sex-biased_analyses/RNAseq_age_group_elasticnet_models_genes_over_2sd_from_mean.tsv",
                           delim = "\t", col_names = T, col_types = cols(.default = "c"))
colnames(rnaseq_genes) <- c("Gene", "Group")
...
all_groups <-  unique(rnaseq_genes$Group)
all_groups <- all_groups[as.numeric(args[1]):as.numeric(args[2])]

for (group in unique(rnaseq_genes$Group)){
  genes_of_interest <- rnaseq_genes %>% 
    filter(Group == group) %>% 
    pull(Gene)
...
output %>% 
  write_delim(paste0(out_path, as.character(args[1]), as.character(args[2]), "RNAseq_elasticnet_age_group_models_significant_GOBP_enrichment.tsv"),
              delim = "\t", col_names = T)
